...
int power(int a, int b, int n) { // a^b mod n    ////вычислить шифротекст
    int tmp = a;
    int sum = tmp;
    for (int i = 1; i < b; i++) {
        for (int j = 1; j < a; j++) {
...
void rsa()
{
    ...
    cin >> p;                                 ///// выбрать два простых различных числа
    cout << "Введите q (простое число): ";
    cin >> q;
    n = p * q;                                /////вычислить модуль (произведение)
    eiler = (p - 1) * (q - 1);                /////вычислить функцию Эйлера
...
void diffie()
{
    ...
    cout << "Введите p (простое число): ";
    cin >> p;
    cout << "Введите g (простое число меньше " << p << "): ";
    cin >> g;
